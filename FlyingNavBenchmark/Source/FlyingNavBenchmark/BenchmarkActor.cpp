...
void ABenchmarkActor::Benchmark()
{
	...
	// Benchmark
	if (BenchmarkDataTable != nullptr && QuerySettings != nullptr && FlyingNavData != nullptr && FlyingNavData->
		GetPathfindingGraph() != nullptr)
	{
		...
		if (FPaths::FileExists(AbsolutePath))
		{
			...
			FString CSVSettings;
			FFileHelper::LoadFileToString(CSVSettings, *AbsolutePath);
			...
			for (int i = 0; i < BenchmarkResolutions.Num(); i++)
			{
				...
				for (int j = 0; j < 3; j++)
				{
					if (PathfindingAlgorithms & (1 << j))
					{
						...
						FSVOPathfindingGraph* const PathfindingGraph = FlyingNavData->GetPathfindingGraph();
...
void ABenchmarkActor::UpdateSettingsPath()
{
	AbsolutePath = FPaths::ConvertRelativePathToFull(SettingsPath.FilePath);
}
#endif // WITH_EDITOR
